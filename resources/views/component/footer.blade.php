    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            ...
            <div class="lg:col-span-1">
                <div class="flex items-center gap-2 mb-6">
                    ...
                    <span class="font-serif font-bold text-xl text-brand-900">Pustaka Aksara</span>
                </div>
                ...
                <div class="flex space-x-4 text-gray-400">
                    @if($footerData && $footerData->social_instagram)
                    <a href="{{ $footerData->social_instagram }}" target="_blank" class="hover:text-brand-700 transition-colors"><i data-lucide="instagram" class="w-5 h-5"></i></a>
                    @endif
                    @if($footerData && $footerData->social_twitter)
                    <a href="{{ $footerData->social_twitter }}" target="_blank" class="hover:text-brand-700 transition-colors"><i data-lucide="twitter" class="w-5 h-5"></i></a>
                    @endif
                    @if($footerData && $footerData->social_facebook)
                    <a href="{{ $footerData->social_facebook }}" target="_blank" class="hover:text-brand-700 transition-colors"><i data-lucide="facebook" class="w-5 h-5"></i></a>
                    @endif
                    @if($footerData && $footerData->social_linkedin)
                    <a href="{{ $footerData->social_linkedin }}" target="_blank" class="hover:text-brand-700 transition-colors"><i data-lucide="linkedin" class="w-5 h-5"></i></a>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4 text-sm text-gray-500">
                    @if($footerData && $footerData->contact_address)
                    ...
                    @if($footerData && $footerData->contact_email)
                    ...
                    @if($footerData && $footerData->contact_phone)
                    <li class="flex items-center gap-3">
        ...
        <div class="border-t border-gray-200 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <p class="text-sm text-gray-400">
                &copy; {{ date('Y') }} Pustaka Aksara. Hak cipta dilindungi undang-undang.
            </p>
